...
function ExorcismStart( event )
	...
	local playerID = caster:GetOwner()
	...
	local unit_name = "npc_dota_tornado"
    ...
    local tornadoradius1  = RotatePosition(origin, anglePos, tornadoradius)
			...
			local unit = CreateUnitByName(unit_name, tornadoradius1, true, caster, caster, caster:GetTeamNumber())
			unit:AddNewModifier(caster, ability, "modifier_kill", {duration = RandomInt(10,60)})
			pfx = ParticleManager:CreateParticle("particles/splash.vpcf", PATTACH_ABSORIGIN_FOLLOW, unit)
			--TO:DO Change Modifer on expire it casts skewer
	...
	unit:OnPhysicsFrame(function(unit)
		...
		if unit.state == "acquiring" then
			...
			if time_between_last_attack >= min_time_between_attacks then
				...
				enemies = FindUnitsInRadius(caster:GetTeamNumber(), source, nil, 10, DOTA_UNIT_TARGET_TEAM_ENEMY, 
...
function deathballs( event )
	local caster = event.caster

print("ded")
end

function spawnmini( event )
	print("spawn mini")
	local caster = event.caster
	local tornado = CreateUnitByName("npc_dota_tornadoactive", caster:GetAbsOrigin(), true, caster, caster, caster:GetTeamNumber())
	local tornadoram = tornado:FindAbilityByName("flytornado")
	if tornadoram ~= nil then
          	...
          	if randomH ~= nil then
          		local heroR = randomH:GetAssignedHero()
          			if heroR:IsAlive() then
          				print('ramming')
          				Timers:CreateTimer(0.3, function()
          				tornado:CastAbilityOnTarget(heroR, tornadoram, -1)
          				end)
          				else return 0.03
          			end
          		else return 0.03
...
function tornado_aura( keys )
	local caster = keys.caster
	local ability = keys.ability
	local target = keys.target
	local aura_damage = ability:GetLevelSpecialValueFor("aura_damage_per_second", (ability:GetLevel() - 1))
	...
	--damage_table.damage_flags = DOTA_DAMAGE_FLAG -- Doesnt trigger abilities and items that get disabled by damage
